...
def get_top_k_stats(attn_scores, k):
    """找到每个 (B, num_heads, T*S) 切片的前 k 个值，并返回最小值和平均值。
    
    注意：这里 k 必须是静态的（Python int）。
    """
    # 如果 k 是 jnp.ndarray，则尝试转换为 Python int（要求 k 在编译时可确定）
    if isinstance(k, jnp.ndarray):
        k = int(k)
    # 使用 jax.lax.top_k 只取前 k 个值
    topk_values, _ = jax.lax.top_k(attn_scores, k)  # shape: (B, num_heads, k)
    min_top_k = topk_values[..., -1]  # 降序排列的最后一个，即第 k 大的值
    mean_top_k = jnp.mean(topk_values, axis=-1)
    return min_top_k, mean_top_k


def apply_dynamic_k_sparsity(
    attn_scores,
    softmax_scores,
    k_threshold=0.92,
    min_k=0.1,
    ultra_min_k=0.05,
    max_k=0.5,
    mean_top_k_threshold=4.0
):
    """
    优化版：假定 attn_scores 形状为 (B, N, T, S)（T：query 长度，S：key 长度），
    这里将 T 和 S 合并为一个维度，一次性对每个 (B, N) 进行 top_k 排序，
    以避免对每个 query chunk 重复排序。

    逻辑与原版一致：
      1. 使用 softmax_scores（经过 clip 处理）计算归一化熵，每个 head 得到 (B, N) 的 global_entropy；
      2. 用全局熵判断：如果熵 < 0.92，则 k_percent = 10%；如果熵 > 0.92，
         则根据 mean_top_k 判断（mean_top_k > 4 则 k_percent = 5%，否则 = 50%）；
      3. 将原始 attn_scores（未 softmax）reshape 成 (B, N, T*S)，对每个 (B, N) 做一次 top_k 排序（静态 k 取 50%）；
      4. 根据动态计算的 k_percent 得到每个 (B, N) 的最终 k（final_k），从排序结果中选出第 final_k 大的值作为阈值；
      5. 用 sigmoid((attn_scores - threshold)/1.0) 得到 soft mask，并与 attn_scores 相乘。
      
    最后输出形状仍为 (B, N, T, S)，数据类型转换为 float32。
    """
    # 先将输入转换为 float16，避免中间重复调用 astype()（减少 HLO 节点）
    attn16 = attn_scores.astype(jnp.float16)
    softmax16 = softmax_scores.astype(jnp.float16)
    B, N, T, S = attn16.shape

    # 1. 基于 softmax16 计算每个 (B, N, T) 上的归一化熵，再对 T 维平均，得到每个 head 的 global_entropy (B, N)
    per_pos_entropy = compute_entropy(softmax16)  # shape: (B, N, T)
    global_entropy = jnp.mean(per_pos_entropy, axis=-1)  # shape: (B, N)

    # 2. 将 attn_scores 合并 T 和 S 两个维度为一个长向量 (B, N, T*S)
    attn_flat = attn16.reshape(B, N, T * S)

    # 3. 先用静态 k（取 50%，即 max_k=0.5）对每个 (B, N) 进行 top_k 排序
    static_k = int(max_k * (T * S))
    static_k = max(1, min(static_k, T * S))  # 确保 k 在 [1, T*S] 范围内
    min_top_k_init, mean_top_k = get_top_k_stats(attn_flat, static_k)  # 两者形状为 (B, N)

    # 4. 根据 global_entropy 与 mean_top_k 计算动态 k 百分比：
    #    当 global_entropy < 0.92 时，k_percent = 10%；当 global_entropy > 0.92 时，
    #    如果 mean_top_k > 4 则 k_percent = 5%，否则 = 50%
    entropy_factor = jax.nn.sigmoid((global_entropy - k_threshold) * 10.0)
    # 默认设为 10%（min_k）
    k_percent = jnp.full_like(global_entropy, min_k)
    mean_factor = jax.nn.sigmoid((mean_top_k - mean_top_k_threshold) * 10.0)
    k_percent = jnp.where(
        entropy_factor > 0.5,
        jnp.where(mean_factor > 0.5, ultra_min_k, max_k),
        min_k
    )  # 形状: (B, N)

    # 5. 根据每个 (B, N) 下的 k_percent 计算最终 k 值 final_k
    final_k = jnp.round(k_percent * (T * S)).astype(jnp.int32)
    final_k = jnp.clip(final_k, 1, T * S)  # (B, N)

    # 6. 对每个 (B, N) 做一次 top_k 排序（排序后结果形状为 (B, N, static_k)）
    topk_values = jax.lax.top_k(attn_flat, static_k)[0]

    # 这里用动态的 final_k 对静态 top_k 结果做索引，获得每个 (B, N) 的阈值
    def select_threshold(topk_slice, k_scalar):
        k_scalar = jnp.clip(k_scalar, 1, static_k)
        return topk_slice[k_scalar - 1]

    # 使用 vmap 分别对 batch 和 head 维度进行动态索引
    threshold_vals = jax.vmap(jax.vmap(select_threshold, in_axes=(0, 0)), in_axes=(0, 0))(topk_values, final_k)
    # threshold_vals 形状为 (B, N)，扩展为 (B, N, 1, 1) 用于广播
    threshold = threshold_vals[..., None, None]

    # 7. 在原始 attn16 上应用 soft mask
    mask = jax.nn.sigmoid((attn16 - threshold) / 1.0)
    sparse_attn = attn16 * mask

    # 最后一次转换为 float32，返回结果形状 (B, N, T, S)
...
class AttentionOp(nn.Module):
  ...
  def qk_product(self, query: jnp.ndarray, key: jnp.ndarray) -> jnp.ndarray:
    ...
    softmax_scores = jax.nn.softmax(attn_scores, axis=-1)

    # 应用稀疏化
    sparse_attn_scores = apply_dynamic_k_sparsity(attn_scores, softmax_scores)

    return sparse_attn_scores.astype(jnp.float32)
