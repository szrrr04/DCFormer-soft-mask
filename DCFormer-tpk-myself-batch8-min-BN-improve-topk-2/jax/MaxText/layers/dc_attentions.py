...
# 优化熵计算：保持基于 softmax_scores 计算
def compute_entropy(softmax_scores):
    """计算熵并归一化到 [0, 1] 范围内，优化版。"""
    B, N, T, S = softmax_scores.shape
    softmax_scores = jnp.clip(softmax_scores, a_min=1e-9, a_max=None)  # 避免 log2 数值问题
    entropy = -jnp.sum(softmax_scores * jnp.log2(softmax_scores), axis=-1)
    
    
    indices = jnp.arange(T)
    valid_keys_count = (indices + 1).astype(jnp.float32) # [1., 2., ..., T]

    # 计算每个位置对应的理论最大熵 log2(有效key数量)
    # 使用 maximum(1.0, ...) 避免 log2(0) 或负数
    max_entropy_precise = jnp.log2(jnp.maximum(1.0, valid_keys_count)) # 形状 (T,)

    # 处理除零情况 (当 t=0, 有效key=1, max_entropy=0 时)
    safe_max_entropy = jnp.maximum(max_entropy_precise, 1e-9) # 形状 (T,)

    # 将 safe_max_entropy 广播到 (B, N, T) 以匹配 entropy 的形状
    # [None, None, :] 会将 (T,) 变成 (1, 1, T), JAX 会自动广播
    safe_max_entropy_broadcastable = safe_max_entropy[None, None, :]

    # 计算精确归一化的熵
    normalized_entropy = entropy / safe_max_entropy_broadcastable
    return normalized_entropy

def apply_dynamic_global_max_mean_sparsity(
    attn_scores: jnp.ndarray,      # 原始注意力分数 (B, N, T, S)
    softmax_scores: jnp.ndarray, # Softmax后分数，用于算熵 (B, N, T, S)
    entropy_threshold: float = 0.92, # 熵阈值
    mean_threshold: float = 4.0,     # 全局均值阈值
    k_low_entropy: float = 0.2,      # 低熵时的 k 系数 (高稀疏)
    k_high_entropy_high_mean: float = 0.1, # 高熵高均值时的 k 系数 (超高稀疏)
    k_high_entropy_low_mean: float = 0.7,  # 高熵低均值时的 k 系数 (低稀疏)
    temperature: float = 1.0,        # Sigmoid 温度
    epsilon: float = 1e-6            # 数值稳定性小量
) -> jnp.ndarray:
    """
    应用基于全局最大值、全局均值和全局熵的动态稀疏化。
    阈值是 per-head global 的，避免了 per-query 阈值可能导致的过拟合。
    阈值计算公式: threshold = global_max - k * (global_max - global_mean)
    系数 k 基于全局熵和全局均值动态选择。

    Args:
        attn_scores: 原始注意力分数 (softmax之前)。形状 (B, N, T, S)。
        softmax_scores: Softmax 后的分数，用于计算熵。形状 (B, N, T, S)。
        entropy_threshold: 熵阈值。
        mean_threshold: 全局均值阈值。
        k_low_entropy: 低熵时使用的 k 系数 (对应约10%保留)。
        k_high_entropy_high_mean: 高熵高均值时使用的 k 系数 (对应约5%保留)。
        k_high_entropy_low_mean: 高熵低均值时使用的 k 系数 (对应约50%保留)。
        temperature: 控制 Sigmoid 软掩码的陡峭度。
        epsilon: 防止数值问题的小常数。

    Returns:
        经过稀疏化处理的注意力分数，形状与输入相同，类型为 float32。
    """
    attn_orig_dtype = attn_scores.dtype
    # 建议在 float32 上计算统计量以保证精度
    compute_dtype = jnp.float32
    attn_compute = attn_scores.astype(compute_dtype)
    softmax_compute = softmax_scores.astype(compute_dtype)
    B, N, T, S = attn_compute.shape

    # 1. 计算全局熵 global_entropy (B, N)
    #    注意：熵的计算基于 softmax 后的分数
    per_pos_entropy = compute_entropy(softmax_compute)  # shape: (B, N, T)
    global_entropy = jnp.mean(per_pos_entropy, axis=-1)  # shape: (B, N)

    # 2. 计算每个头的全局均值和全局最大值
    #    注意：均值和最大值基于原始 attn_scores 计算
    attn_flat = attn_compute.reshape(B, N, T * S)

    valid_mask = attn_flat > (DEFAULT_MASK_VALUE * 0.9)
    # --- 改进点3: 修正 Global Max 计算 ---
    global_max = jnp.max(jnp.where(valid_mask, attn_flat, -jnp.inf), axis=-1)
    global_max = jnp.nan_to_num(global_max, nan=-jnp.inf, posinf=jnp.inf, neginf=-jnp.inf)

    # --- 改进点2: 修正 Global Mean 计算 ---
    masked_sum = jnp.sum(jnp.where(valid_mask, attn_flat, 0.), axis=-1)
    valid_count = jnp.sum(valid_mask, axis=-1)
    global_mean = masked_sum / jnp.maximum(valid_count, 1) # 避免除以零

    # 3. 根据全局熵和全局均值动态选择 k 系数
    #    条件: 低熵 (< threshold)
    is_low_entropy = global_entropy < entropy_threshold
    #    条件: 高熵 (>= threshold) 且 高均值 (> threshold)
    is_high_entropy_high_mean = (~is_low_entropy) & (global_mean > mean_threshold)
    #    条件: 高熵 (>= threshold) 且 低均值 (<= threshold)
    #    (这个条件是剩下的一种情况，可以用 jnp.where 嵌套或逻辑运算实现)

    #    选择 k 系数 (三层嵌套 jnp.where)
    k_coeff = jnp.where(
        is_low_entropy,
        k_low_entropy, # 情况1: 低熵
        jnp.where(
            is_high_entropy_high_mean,
            k_high_entropy_high_mean, # 情况2: 高熵高均值
            k_high_entropy_low_mean   # 情况3: 高熵低均值
        )
    ) # Shape: (B, N)

    # 4. 计算每个头的全局阈值 threshold
    #    增加 epsilon 防止 max == mean 时出现问题
    range_val = jnp.maximum(global_max - global_mean, epsilon)
    global_threshold = global_max - k_coeff * range_val # Shape: (B, N)

    # 5. 将全局阈值广播到 (B, N, 1, 1)
    threshold_broadcast = global_threshold[:, :, None, None]

    # 6. 计算软掩码 (在原始数据类型或 compute_dtype 上进行)
    #    使用 attn_scores 的原始类型进行掩码计算可能更高效
    attn_mask_compute = attn_scores.astype(jnp.promote_types(attn_orig_dtype, jnp.float16)) # 或直接用 attn_orig_dtype
    safe_temperature = jnp.maximum(jnp.asarray(temperature, dtype=attn_mask_compute.dtype), epsilon)
    mask = jax.nn.sigmoid((attn_mask_compute - threshold_broadcast.astype(attn_mask_compute.dtype)) / safe_temperature)

    # 7. 应用掩码得到稀疏分数
    sparse_attn = attn_mask_compute * mask

    # 8. 转换回 float32 (根据原始代码和后续流程要求)
    return sparse_attn.astype(jnp.float32)
...
class AttentionOp(nn.Module):
  ...
  sparsity_entropy_threshold: float = 0.92
  sparsity_mean_threshold: float = 4.0
  sparsity_k_low_entropy: float = 0.2      # 需调优
  sparsity_k_high_entropy_high_mean: float = 0.1 # 需调优
  sparsity_k_high_entropy_low_mean: float = 0.7  # 需调优
  sparsity_temperature: float = 1.0
  ...
  def _apply_attention_dot(
    ...
    # bnts
    attn_weights = self.qk_product(query, key, attn_mask)
  ...
  def qk_product(self, query: jnp.ndarray, key: jnp.ndarray, attn_mask: Array | None) -> jnp.ndarray:
        """Query-Key 乘积并应用基于全局Max/Mean/Entropy的动态稀疏化。"""
        # 计算点积，得到原始的注意力分数
        # 为了效率，可以在较低精度计算，但后续统计量建议在 float32
        attn_scores = jnp.einsum('btnd,bsnd->bnts', query.astype(jnp.float16), key.astype(jnp.float16))

        compute_dtype = jnp.float32
        attn_scores_compute = attn_scores.astype(compute_dtype)

        if attn_mask is not None:
            masked_attn_scores = apply_mask_to_logits(attn_scores_compute, attn_mask.astype(compute_dtype))
        else:
            masked_attn_scores = attn_scores_compute

        # 为了计算熵，需要 softmax 后的分数
        # 为了稳定性，softmax 通常在 float32 上计算
        softmax_scores_masked = jax.nn.softmax(masked_attn_scores, axis=-1)

        # 应用新的稀疏化函数
        # 注意：attn_scores 传入可能是 float16, softmax_scores 是 float32
        sparse_masked_attn_scores = apply_dynamic_global_max_mean_sparsity(
            masked_attn_scores,              # 传入已屏蔽的分数
            softmax_scores_masked,           # 传入基于已屏蔽分数的 Softmax
            entropy_threshold=self.sparsity_entropy_threshold, # 阈值暂时不变
            mean_threshold=self.sparsity_mean_threshold,
            k_low_entropy=self.sparsity_k_low_entropy,
            k_high_entropy_high_mean=self.sparsity_k_high_entropy_high_mean,
            k_high_entropy_low_mean=self.sparsity_k_high_entropy_low_mean,
            temperature=self.sparsity_temperature
        )

        # sparse_attn_scores 已经是 float32，后续根据 float32_qk_product 决定是否需要转换
        return sparse_masked_attn_scores
