...
def compute_entropy(softmax_scores):
    ...
    max_entropy = math.log2(softmax_scores.shape[-1])
...
def apply_dynamic_global_max_mean_sparsity(
    ...
    global_mean = jnp.mean(attn_flat, axis=-1) # Shape: (B, N)
    global_max = jnp.max(attn_flat, axis=-1)   # Shape: (B, N)
...
class AttentionOp(nn.Module):
  ...
  def qk_product(self, query: jnp.ndarray, key: jnp.ndarray) -> jnp.ndarray:
        ...
        softmax_scores = jax.nn.softmax(attn_scores.astype(jnp.float32), axis=-1)

        # 应用新的稀疏化函数
        # 注意：attn_scores 传入可能是 float16, softmax_scores 是 float32
        sparse_attn_scores = apply_dynamic_global_max_mean_sparsity(
            attn_scores,                     # 原始分数 (可能是 float16)
            softmax_scores,                  # Softmax后分数 (float32)
            entropy_threshold=self.sparsity_entropy_threshold,
        ...
        if not self.float32_qk_product:
             # 如果后续操作明确不需要 float32 (虽然通常需要以保证 pre_compose/softmax 稳定)
             # 可以考虑转回 query.dtype，但这可能引入额外开销和精度损失
             # return sparse_attn_scores.astype(query.dtype) # 谨慎使用
             # 通常保持 float32 是安全的
             return sparse_attn_scores
        else:
             # 保持 float32
             return sparse_attn_scores
